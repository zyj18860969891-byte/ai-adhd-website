...
class CustomMCPServer {
  ...
  // 格式化Zod错误
  private formatZodError(zodError: any, toolName: string) {
    ...
    return {
      ...
      tool: toolName,
      issues: errorDetails,
      suggestions: errorDetails.map((e: any) => e.suggestion),
      example: this.getToolExample(toolName)
    };
  }

  // 获取工具示例
  private getToolExample(toolName: string) {
  ...
  // 处理初始化请求
  public async handleInitialize(params: any) {
  ...
  // 处理工具列表请求
  public async handleToolsList(params: any) {
    ...
    try {
      ...
      try {
        switch (toolName) {
          ...
          default:
            throw new Error(`工具 ${toolName} 不存在。可用工具: ${(await this.getToolList()).map((t: any) => t.name).join(', ')}`);
        }
  ...
  public async start() {
    ...
    process.stdin.on('data', async (data) => {
      try {
        ...
        console.log(`[INFO] Sending response for ID: ${response.id}`);
